            ...
            MobilnaBankaTheme {
                ...
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    NavHost(
                        ...
                        composable("google_map_screen") { GoogleMapScreen() }
                    }
                }
                if (showDialog) {
...
fun BankAccountScreen(innerPadding: PaddingValues, bankAccountViewModel: BankAccountViewModel, navController: NavHostController) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(
                onClick = { navController.navigate("google_map_screen") },
                modifier = Modifier.fillMaxWidth().padding(top = 16.dp)
            ) {
                Text(text = "Odpri Google Zemljevide")
            }
        }
    }
}
...
fun GoogleMapScreen() {
    ...
    AndroidView(
        ...
        map.getMapAsync { googleMap ->
            ...
            val location = LatLng(46.056946, 14.505751) // Lokacija za prikaz (primer: Ljubljana)
            googleMap.addMarker(
                MarkerOptions().position(location).title("Marker v Ljubljani")
            )
